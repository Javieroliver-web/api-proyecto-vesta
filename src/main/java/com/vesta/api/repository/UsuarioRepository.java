...
public interface UsuarioRepository extends JpaRepository<Usuario, Long> {
        ...
        @org.springframework.data.jpa.repository.Query("SELECT u FROM Usuario u WHERE " +
                        "(u.datosEliminados = false OR u.datosEliminados IS NULL) AND " +
                        "(:keyword IS NULL OR :keyword = '' OR " +
                        " LOWER(u.email) LIKE LOWER(CONCAT('%', :keyword, '%')) OR " +
                        " LOWER(u.nombreCompleto) LIKE LOWER(CONCAT('%', :keyword, '%'))) AND " +
                        "(:role IS NULL OR :role = '' OR u.rol = :role) AND " +
                        "(:status IS NULL OR :status = '' OR " +
                        " (:status = 'true' AND (u.activo = true OR u.activo IS NULL)) OR " +
                        " (:status = 'false' AND u.activo = false))")
